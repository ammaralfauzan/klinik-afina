...
type Pengaturan = { id: number; nama_klinik: string; alamat: string; telepon: string; jam_buka: string; jam_tutup: string; dokter_jaga: string; };
...
export default function PengaturanPage() {
  ...
  useEffect(() => {
    supabase.from("pengaturan").select("*").eq("id", 1).single().then(({ data }) => { if (data) setForm(data); });
  }, []);
